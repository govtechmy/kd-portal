...
const StatisticsCharts: FC<StatisticsChartsProps> = ({ visitorData, countryData, referrerData, deviceData, browserData, pageData, newsData, locale, aggregatedData }) => {
  ...
  const processedVisitorData = useMemo(() => {
    ...
    return visitorData.map((item, index) => ({
      ...
      cumulative: visitorData.slice(0, index + 1).reduce((sum, d) => sum + d.total_visitors, 0)
    }));
  }, [visitorData]);
  ...
  const filteredData = useMemo(() => {
    ...
    const cutoffDate = new Date(now.getTime() - (rangeDays[selectedRange] * 24 * 60 * 60 * 1000));
  ...
  const comparisonMetrics = useMemo(() => {
    ...
    const currentAvg = currentPeriod.reduce((sum, item) => sum + item.visitors, 0) / currentPeriod.length;
    const previousAvg = previousPeriod.reduce((sum, item) => sum + item.visitors, 0) / previousPeriod.length;
  ...
  const processedBrowserData = useMemo(() => {
    ...
    const browserConfig: Record<string, { icon: React.ReactNode; color: string }> = {
      "Chrome": { 
  ...
  const processedPageData = useMemo(() => {
    ...
    return pageData
      ...
      .map((item) => ({
        name: item.page_path.length > 15 ? item.page_path.substring(0, 15) + '...' : item.page_path,
  ...
  const processedNewsData = useMemo(() => {
    ...
    return newsData
      ...
      .map((item) => ({
        name: item.news_slug.length > 25 ? item.news_slug.substring(0, 25) + '...' : item.news_slug,
      ...
      return (
        <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            {data.visits.toLocaleString()} {t("Statistics.chart.visitors")} ({data.percentage.toFixed(1)}%)
          </p>
        </div>
      );
    }
  ...
  const processedAggregatedData = useMemo(() => {
    ...
    switch (selectedRange) {
      case '7d':
        filteredData = aggregatedData.filter(item => new Date(item.day) >= getDaysAgo(7));
        break;
      case '30d':
        filteredData = aggregatedData.filter(item => new Date(item.day) >= getDaysAgo(30));
        break;
      case '90d':
        filteredData = aggregatedData.filter(item => new Date(item.day) >= getDaysAgo(90));
        break;
      case '1y':
        filteredData = aggregatedData.filter(item => new Date(item.day) >= getDaysAgo(365));
        break;
    }
    ...
    switch (selectedDataType) {
      case 'referrers':
        return {
          ...
          areas: [
            { key: 'direct_visits', color: '#10B981', label: t("Statistics.traffic_sources.direct") },
            { key: 'google_visits', color: '#4285F4', label: t("Statistics.traffic_sources.google") },
            { key: 'bing_visits', color: '#00A1F1', label: t("Statistics.traffic_sources.bing") },
            { key: 'facebook_visits', color: '#1877F2', label: t("Statistics.traffic_sources.facebook") },
            { key: 'linkedin_visits', color: '#0A66C2', label: t("Statistics.traffic_sources.linkedin") },
            { key: 'internal_visits', color: '#8B5CF6', label: t("Statistics.traffic_sources.internal") },
            { key: 'other_referrals', color: '#F59E0B', label: t("Statistics.traffic_sources.other") },
          ]
        };
      case 'locations':
        return {
    ...
    <div className="space-y-8">
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {chartConfig.areas.map((area) => (
              <div key={area.key} className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full flex-shrink-0" style={{ backgroundColor: area.color }}></div>
                <span className="text-xs sm:text-sm text-gray-600 truncate">{area.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
        <div className="mb-4 sm:mb-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.visitor_chart.title")}</h3>
              <p className="text-sm text-gray-600">{t("Statistics.visitor_chart.subtitle")}</p>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
              <span className="text-sm font-medium text-gray-500">{t("Statistics.time_range.label")}:</span>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="text-lg sm:text-2xl font-bold text-gray-900">{totalVisitors.toLocaleString()}</div>
            <div className="text-xs sm:text-sm text-gray-600">{t("Statistics.metrics.current_visitors")}</div>
          </div>
          <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="text-lg sm:text-2xl font-bold text-gray-900">{totalUnique.toLocaleString()}</div>
            <div className="text-xs sm:text-sm text-gray-600">{t("Statistics.metrics.unique_visitors")}</div>
          </div>
          <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="text-lg sm:text-2xl font-bold text-gray-900">{cumulativeVisitors.toLocaleString()}</div>
            <div className="text-xs sm:text-sm text-gray-600">{t("Statistics.metrics.cumulative")}</div>
          </div>
          <div className="text-center p-3 sm:p-4 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-center gap-1 sm:gap-2">
              {getTrendIcon(comparisonMetrics.trend)}
              <span className={`text-base sm:text-lg font-bold ${getTrendColor(comparisonMetrics.trend)}`}>
                {comparisonMetrics.change > 0 ? '+' : ''}{comparisonMetrics.change}%
              </span>
            </div>
            <div className="text-xs sm:text-sm text-gray-600">{t("Statistics.metrics.vs_previous_week")}</div>
          </div>
        </div>
        ...
        {filteredData.length > 0 ? (
              ...
              <Tooltip
                ...
                formatter={(value: number, name: string) => [
                  <span style={{ color: "#3B82F6", fontWeight: "bold" }}>
                    {t("Statistics.chart.daily_visitors")}: {value.toLocaleString()}
                  </span>
                ]}
                labelFormatter={(label) => (
                  <span style={{ color: "#1F2937", fontWeight: "600" }}>
          ...
          <div className="h-48 sm:h-64 flex items-center justify-center text-gray-500">
            <div className="text-center">
              <p className="text-sm">{t("Statistics.visitor_chart.no_data_range")}</p>
              <p className="text-xs mt-1">{t("Statistics.chart.data_source")}: <a href="https://www.tinybird.co/docs/api-reference" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">{t("Statistics.chart.tinybird_api")}</a></p>
            </div>
          </div>
        )}
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
          <span>{t("Statistics.chart.data_source")}: {visitorData.length > 0 ? <a href="https://www.tinybird.co/docs/api-reference" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">{t("Statistics.chart.tinybird_api")}</a> : t("Statistics.chart.no_data")}</span>
          <span>{t("Statistics.chart.showing_points", { count: filteredData.length })}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.pages.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.chart.visitors").toUpperCase()}</span>
          </div>
            ...
            <ResponsiveContainer width="100%" height={300} className="sm:h-[350px]">
              <BarChart data={processedPageData}>
                ...
                <Tooltip
                  ...
                  labelFormatter={(label) => {
                    const fullName = processedPageData.find(item => item.name === label)?.fullName || label;
                    return (
                      <span style={{ color: "#1F2937", fontWeight: "600" }}>
                        {fullName}
                      </span>
                ...
                >
                  {processedPageData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill="#EBF4FF" stroke="#3B82F6" />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="mt-4 space-y-2">
            {processedPageData.map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-600">{item.hits.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.news.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.news.hits").toUpperCase()}</span>
          </div>
            ...
            <ResponsiveContainer width="100%" height={300} className="sm:h-[350px]">
              <BarChart data={processedNewsData}>
                ...
                <Tooltip
                  ...
                  labelFormatter={(label) => {
                    const fullName = processedNewsData.find(item => item.name === label)?.fullName || label;
                    return (
                      <span style={{ color: "#1F2937", fontWeight: "600" }}>
                        {fullName}
                      </span>
                ...
                >
                  {processedNewsData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill="#D1FAE5" stroke="#10B981" />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          {processedNewsData.length > 0 && (
            <div className="mt-4 space-y-2">
              <h4 className="text-sm font-medium text-gray-700">{t("Statistics.news.legend_title")}:</h4>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.traffic_sources.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.chart.visitors").toUpperCase()}</span>
          </div>
          {processedReferrerData.length > 0 ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="text-center max-w-[80px]">
                    <p className="text-sm font-semibold text-gray-900 mb-1 leading-tight">
                      {processedReferrerData.find(item => item.name === hoveredReferrer)?.name}
                    </p>
                    <p className="text-xs text-gray-600">
                      {processedReferrerData.find(item => item.name === hoveredReferrer)?.percentage.toFixed(1)}%
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="h-48 sm:h-64 flex items-center justify-center text-gray-500">
              <p className="text-sm">{t("Statistics.traffic_sources.no_data")}</p>
            </div>
          )}
          <div className="mt-4 space-y-2">
            {processedReferrerData.map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-gray-600 font-medium">{item.visits.toLocaleString()}</span>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.locations.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.chart.visitors").toUpperCase()}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.browsers.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.chart.users").toUpperCase()}</span>
          </div>
            ...
            <div className="relative">
              ...
              {hoveredBrowser && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="text-center max-w-[80px]">
                    <p className="text-sm font-semibold text-gray-900 mb-1 leading-tight">
                      {processedBrowserData.find(item => item.name === hoveredBrowser)?.name}
                    </p>
                    <p className="text-xs text-gray-600">
                      {processedBrowserData.find(item => item.name === hoveredBrowser)?.percentage.toFixed(1)}%
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="mt-4 space-y-2">
            {processedBrowserData.map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-gray-600 font-medium">{item.users.toLocaleString()}</span>
                  <span className="text-xs text-gray-400">({item.percentage.toFixed(1)}%)</span>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 sm:p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">{t("Statistics.devices.title")}</h3>
            <span className="text-sm font-medium text-gray-500">{t("Statistics.chart.users").toUpperCase()}</span>
          </div>
          ...
          <div className="mt-4 space-y-2">
            {processedDeviceData.map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                ...
                <span className="text-gray-600">{item.users.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
