...
import PageStatisticsCard from "@/components/statistik/page-statistics";
...
const StatistikComponent: FC<StatistikProps> = ({ data, locale }) => {
  ...
  const StatCard = ({ title, metrics }: { title: string; metrics: Array<{ label: string; value: string; change: string }> }) => (
    ...
    <main>
      ...
      <Section>
        <div className="relative gap-6 border-washed-100 max-lg:pt-12 lg:grid lg:grid-cols-12 lg:border-x lg:px-6">
          <div className="left-0 top-16 col-span-5 flex max-w-full flex-col items-start gap-y-4.5 lg:sticky lg:h-fit lg:py-[84px] xl:col-span-4">
            <h2 className="font-poppins text-[1.5rem]/8 font-semibold sm:text-2xl">
              {t("Statistics.page_stats_title")}
            </h2>
            <p className="text-sm text-black-700">
              {t("Statistics.page_stats_description")}
            </p>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-lg bg-blue-50 p-6">
            <h3 className="mb-2 text-lg font-semibold text-blue-900">
              {t("Statistics.important_note")}
            </h3>
            <p className="text-blue-800">
              {t("Statistics.note_text")}
            </p>
          </div>
        </div>
      </Section>
